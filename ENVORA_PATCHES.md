...
- Changed files: `mujoco_warp/_src/collision_core.py`,
  `mujoco_warp/_src/collision_driver.py`, and
...
6. Broad phase records a compact index of pairs that actually contain an SDF,
   so ordinary primitive and convex contacts do not expand into
   `sdf_initpoints` no-op narrow-phase threads.
7. CUDA graph conditionals skip the SDF kernel entirely on steps without an
   SDF collision candidate.
8. The SDF kernel uses a fixed persistent grid and strides over the compact
   work list instead of launching `sdf_initpoints * naconmax` threads.
...
2. Reapply only the changes described above to `collision_core.py`,
   `collision_driver.py`, and `collision_sdf.py`; do not copy complete files
